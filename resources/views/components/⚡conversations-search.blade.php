...
new class extends Component {
    ...
    public function selectConversation($conversationId)
    {
        ...
        broadcast(new MessageRead($this->selectedConversationId));
    }
    ...
    public function sendMessage()
    {
        ...
        Message::create([
            ...
            'sender_id' => Auth::id(),
            'receiver_id' => $this->conversation->user1_id == Auth::id() ? $this->conversation->user2_id : $this->conversation->user1_id,
        ...
        broadcast(new SendMessage($this->selectedConversationId, $this->message));
        broadcast(new NewMessageRecieved($this->conversation->user1_id == Auth::id() ? $this->conversation->user2_id : $this->conversation->user1_id));
        $this->message = "";
    }
    ...
    public function messageSent()
    {
        if ($this->selectedConversationId){
            broadcast(new MessageRead($this->selectedConversationId));
            $this->conversation->fresh()->load('messages');
        }
    ...
    <aside id="sidebar" class="chat-sidebar flex flex-col glass z-20">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            @forelse ($this->conversations as $conversation)
                <div class="flex items-center p-3 space-x-4 cursor-pointer {{ $conversation->id == $selectedConversationId ? 'bg-indigo-500/10' : 'hover:bg-white/5' }} rounded-xl transition-all"
                    ...
                    <div class="relative">
                        ...
                        <div
                            class="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 border-2 border-slate-900 rounded-full">
                        </div>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-baseline">
                            ...
                            <span
                                class="text-xs text-slate-500">{{ $conversation->lastMessage?->created_at->format('h:i A') ?? "N/A"}}</span>
                        </div>
                        <p class="text-xs text-slate-400 truncate mt-0.5">
                            {{ $conversation->lastMessage->message ?? 'No messages' }}</p>
                    </div>
        ...
        <div class="absolute bottom-0 left-0 right-0 w-full p-4 border-t border-white/5 bg-slate-900/50">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <a class="p-2 hover:bg-slate-500/5 rounded-lg transition-colors cursor-pointer" id="settings-toggle">
                        <i data-lucide="settings" class="w-5 h-5 text-slate-400"></i>
                    </a>
                    <div id="settings" class="hidden absolute bottom-20 right-4 w-32 glass rounded-lg p-2">
                        <a class="p-2 hover:bg-blue-500/5 rounded-lg transition-colors flex items-center space-x-2" href="{{ route('profile.edit') }}">
                            <i data-lucide="user" class="w-5 h-5 text-blue-400"></i>
                            <span class="text-sm text-blue-400 cursor-pointer">Profile</span>
                        </a>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="p-2 hover:bg-red-500/5 rounded-lg transition-colors flex items-center space-x-2">
                                <i data-lucide="log-out" class="w-5 h-5 text-red-400"></i>
                                <span class="text-sm text-red-400 cursor-pointer">Logout</span>
                            </button>
                        </form>
                    </div>
                </div>
        ...
        @if($this->conversation)
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="font-bold text-lg leading-tight">
                            {{ $this->conversation->user1_id == Auth::id() ? $this->conversation->user2->name : $this->conversation->user1->name }}
                        </h2>
            ...
            <div class="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar" id="messages-container">
                ...
                <div class="flex justify-center">
                    <span
                        class="px-4 py-1.5 bg-slate-900/50 border border-white/5 rounded-full text-[10px] uppercase tracking-wider font-semibold text-slate-500">
                        Today, Oct 12
                    </span>
                </div>
                        ...
                        @foreach($this->conversation->messages as $message)
                            @if($message->sender_id != auth()->id())
                                ...
                                <div class="flex items-end space-x-3 max-w-[80%]">
                                    ...
                                    <div class="space-y-1">
                                        <div
                                            class="message-bubble-received p-4 rounded-2xl text-sm leading-relaxed text-slate-200 shadow-sm">
                                            {{ $message->message }}
                                        </div>
                            ...
                            @else
                                ...
                                <div class="flex items-end space-x-3 max-w-[80%] ml-auto flex-row-reverse">
                                    ...
                                    <div class="space-y-1 text-right">
                                        <div
                                            class="message-bubble-sent p-4 rounded-2xl text-sm leading-relaxed text-white shadow-lg shadow-indigo-500/20">
                                            {{ $message->message }}
                                        </div>
            ...
            <form wire:submit.prevent="sendMessage">
                <div class="p-6 bg-slate-950/30">
                    <div class="max-w-4xl mx-auto relative group">
                        <div
                            class="absolute inset-0 bg-indigo-500/5 blur-2xl rounded-full opacity-0 group-focus-within:opacity-100 transition-opacity">
                        </div>
                        <div class="relative glass rounded-2xl p-2 flex items-end space-x-2 border-white/10 shadow-2xl">
                            ...
                            <textarea wire:model.live="message" wire:key="{{ $message }}" rows="1"
                                placeholder="Type a message..."
                                class="flex-1 bg-transparent border-none focus:ring-0 py-3 text-sm text-slate-200 placeholder-slate-500 resize-none max-h-20 custom-scrollbar"></textarea>
                            <div class="flex items-center p-1 space-x-1">
                                ...
                                @if ($this->message)
                                    <button type="submit"
                                        class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-xl transition-all shadow-lg shadow-indigo-600/30 group">
                                        Send
                                    </button>
                                @endif
                            </div>
...
<script>
    ...
    Echo.private(`user.{{Auth::id()}}`)
        .listen('.NewMessageRecieved', (e) => {
            this.messageSent();
        });

    this.$on('conversation-selected', ({ conversationId }) => {
